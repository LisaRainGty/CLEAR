...
def command_signature(job_name, command):
    blob = json.dumps(
        {"name": job_name, "command": tuple(command)}, ensure_ascii=False)
    return hashlib.sha256(blob.encode("utf-8")).hexdigest()


def normalize_command(command):
    """Make a recorded command portable without weakening its semantics."""
    normalized = []
    path_roots = ("configs", "data", "embeddings", "paper", "results", "scripts", "src")
    for index, raw in enumerate(command):
        value = str(raw)
        if index == 0 and (value.endswith("/python") or value.endswith("/python3")):
            normalized.append("<PYTHON>")
            continue
        mapped = None
        unix_value = value.replace("\\", "/")
        for marker in path_roots:
            needle = f"/{marker}/"
            if needle in unix_value:
                mapped = marker + "/" + unix_value.split(needle, 1)[1]
                break
        if mapped is not None:
            normalized.append(mapped)
        elif (unix_value.endswith("bge-large-zh-v1___5")
              or unix_value == "BAAI/bge-large-zh-v1.5"):
            normalized.append("BAAI/bge-large-zh-v1.5")
        else:
...
def load_rows(root: Path, result_root: Path, evidence_policy: str,
              evidence_policy_overrides=None, expected_commands=None,
              external_tags=None, excluded_tags=None):
    evidence_policy_overrides = evidence_policy_overrides or {}
    expected_commands = expected_commands or {}
    external_tags = set(external_tags or ())
    ...
    for path in sorted((result_root / "jobs").glob("*.jsonl")):
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip():
                ...
                suite_job = str(row.get("_suite_job") or path.stem)
                if row_tag in excluded_tags:
                    reasons.append("explicitly_excluded_legacy_result")
                elif row_tag not in external_tags:
                    expected_command = expected_commands.get(suite_job)
                    status_path = result_root / "status" / f"{suite_job}.json"
                    status = load_json(status_path)
                    if expected_command is None:
                        reasons.append("job_not_in_final_suite")
                    elif not status or status.get("returncode") != 0:
                        reasons.append("missing_success_status")
                    elif status.get("signature") != command_signature(
                            suite_job, status.get("command", ())):
                        reasons.append("invalid_status_signature")
                    elif normalize_command(status.get("command", ())) != \
                            normalize_command(expected_command):
                        reasons.append("stale_job_command")
...
def main():
    ...
    # A row from an older architecture can share the same dataset and tag.
    # Accept current-suite rows only when their success record matches the
    # exact command signature generated by this locked config.
    import run_paper_suite as paper_suite
    paper_suite.configure(config_path)
    expected_jobs = paper_suite.build_jobs({
        "audit", "table3", "ablation", "hparams", "xdom", "analysis",
    })
    expected_commands = {job.name: job.command for job in expected_jobs}
    excluded_tags = (
        {"qwen2p5_7b_qlora_sft"}
        if config.get("paper_suite", {}).get("exclude_legacy_qwen", False)
        else set()
    )
    rows, rejected_rows = load_rows(
        root, result_root, policy, evidence_policy_overrides,
        expected_commands=expected_commands,
        external_tags=SINGLE_RUN_TAGS,
        excluded_tags=excluded_tags)
    agg = aggregate(rows)
    sft_namespace = str(config.get("paper_suite", {}).get(
        "supervised_llm_result_namespace", ""))
    sft_summary = load_json(
        root / "results" / sft_namespace / "llm_sft_v2_final_summary.json"
    ) if sft_namespace else None
    if (sft_summary
            and sft_summary.get("selection_test_metrics_accessed") is False
            and sft_summary.get("dataset_sha256") == config["dataset"]["sha256"]
            and sft_summary.get("evidence_policy") == policy
            and sft_summary.get("standard_deviation") == "sample ddof=1"):
        agg["qwen2p5_7b_qlora_sft_locked"] = {
            "n": len(sft_summary.get("runs", [])),
            "seeds": list(sft_summary.get("seeds", [])),
            **sft_summary.get("metrics", {}),
        }
    ...
    metric_table(lines, "Table 3. In-domain main comparison", (
        ...
        ("Qwen2.5-7B QLoRA SFT (locked)", "qwen2p5_7b_qlora_sft_locked"),
    ...
    hp_entries = (
        ...
        *fusion_hp_entries, ("Attention heads=4", "hp_heads4"),
